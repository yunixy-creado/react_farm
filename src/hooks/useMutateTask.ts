...
export const useMutateTask = () => {
  ...
  const createTaskMutation = useMutation(
    (task: Omit<Task, 'id'>) =>
      axios.post<Task>(`${process.env.REACT_APP_API_URL}api/todo`, task, {
  ...
  const updateTaskMutation = useMutation(
    (task: Task) =>
      axios.put<Task>(
        `${process.env.REACT_APP_API_URL}api/todo/${task.id}`,
        {
          title: task.title,
  ...
  const deleteTaskMutation = useMutation(
    (id: string) =>
      axios.delete(`${process.env.REACT_APP_API_URL}api/todo/${id}`, {
        withCredentials: true,
      }),
    {
      onSuccess: (res, variables) => {
